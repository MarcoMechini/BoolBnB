...
const schemaValidazione = yup.object().shape({
    id_property: yup.number().typeError("Devi inserire un numero").required("Devi scegliere una tipologia di immobile").positive("Devi scegliere una tipologia di immobile").integer("Deve essere un numero intero"),
    title: yup.string().min(3, "il titolo deve avere minimo di tre lettere").max(255, "È troppo lungo").matches(/[a-zA-Z]/, "Il titolo deve contenere almeno una lettera").required("Inserire il titolo della casa"),
    city: yup.string().min(3, "La città deve avere minimo tre lettere").max(100, "È troppo lungo").matches(/^[A-Za-z]+$/, "La città deve contenere solo lettere").required("Inserire la città"),
    descr: yup.string().nullable().max(500, "È troppo lungo"),
    rooms: yup.number().typeError("Specificare il numero di stanze in numeri").required("Inserire numero stanze").positive("bisogna inserire almeno una stanza").integer("non deve avere punti o virgole").min(1, "Deve avere almeno una stanza"),
    bedrooms: yup.number().typeError("Devi specificare i posti letto con un numero").required("Inserire il numero letti").positive("Deve essere positivo").integer("Deve avere almeno un letto").min(1, "Deve avere almeno un letto")
    ...
    bathrooms: yup.number().typeError("Devispecificare i bagni con un numero").required("Inserire il numero dei bagni").positive("Deve avere almeno un bagno").integer("Deve essere un numero intero").min(1, "Deve avere almeno un bagno"),
    square_meters: yup.number().typeError("Devispecificare i MQ con un numero").required("Inserire metri quadrati").positive("Deve essere positivo").integer("Deve essere un numero intero").min(10, "Deve essere almeno 10 metri quadri"),
    address: yup.string().required("Inserire la via").min(5, "La Via deve essere minimo di 5 caratteri").max(255, "È troppo lungo"),
            ...
            <section className="container">
                <form className={`${style.formInserimento}`}>
                    <div className={`${style.rowForm}`}>
                        ...
                        {errors.title && <p className={`${style.error}`}>{errors.title}</p>}</div>
                        <div className={`${style.rowForm}`}>
